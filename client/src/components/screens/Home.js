                ...
                data.map(item => {
                    return (
                        <div className="card home-card" key={item._id}>
                            <div>
                            <img alt="ava" style={{ width:"50px",objectFit:"cover", height:"50px",float:"left", borderRadius:"50%",margin:"10px 0 10px 10px" }}
                            src={item.postedBy.pic}
                    />
                                <h5 style={{float:"left"}}><Link style={{margin:"15px 0px 15px 0"}} to={ item.postedBy._id == state._id ? `/profile` :`/profile/${item.postedBy._id}`}>{item.postedBy.name}</Link></h5>
                                {
                                    item.postedBy._id == state._id
                                    &&
                                    <i onClick={()=>deletePost(item._id)} className="material-icons" style={{ color: "red",float:"right",margin:"15px" }}>delete</i>    
                                }
                            </div>
